...
const Image = ({ url, showButtuon, onChangePhoto }) => {
  useEffect(() => {
    (async () => {
      if (Platform.OS !== "web") {
        const { status } =
        ...
        if (status !== "granted") {
          Alert.alert("Photo Permission, Need Camera Permission");
        }
      }
    })();
  }, []);
  ...
  return (
    <Container>
      ...
      {showButtuon && <PhotoButton onPress={_handlePhotoBtnPress} />}
    </Container>
  );
};
